...
export function setupBlurb(alertType?: string): string {
  ...
  const BLURB: Record<string, string> = {
    ...
    orb_break: "A 15m close broke a key rail — the opening range (ORH/ORL) or a prior-day high/low (PDH/PDL).",
    orb_held: "A key rail — opening range or prior-day level — tested from the far side and held. The direction tells you the side.",
    orb_retest: "A broken rail was retested and rejected — the second-entry continuation.",
    orb_exit: "A key rail was given back on a confirmed 15m close — the exit / stop.",
