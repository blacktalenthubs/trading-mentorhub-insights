...
/* ── Short allowlist — which stocks get short signals (empty = whole watchlist) ── */
function ShortAllowlistSection() {
  const { data, isError } = useShortAllowlist();
  const { data: watchlist } = useWatchlist();
  const update = useUpdateShortAllowlist();
  const [input, setInput] = useState("");
  if (isError) return null;
  ...
  return (
    <Section title="Short signals — which stocks" icon={<TrendingDown className="h-4 w-4 text-bearish-text" />}>
      <p className="text-[12px] leading-relaxed text-text-muted mb-3">
        Short setups (PDL break, PDH rejection, PDH failed break) are <b>off by default</b> — turn them on in <b>Alert Types</b> above. Once on: leave this list <b>empty</b> to get shorts across <b>your whole watchlist</b>, or add symbols to get shorts <b>only</b> on those names.
      </p>
      <div className="mt-1">
        <div className="mb-1.5 flex items-baseline justify-between">
          <label className="text-[11px] font-semibold uppercase tracking-wide text-text-faint">Short only these (empty = whole watchlist)</label>
          {symbols.length > 0 && <span className="text-[10px] text-text-faint">{symbols.length}</span>}
        </div>
        {symbols.length > 0 ? (
          <div className="mb-2 flex flex-wrap gap-1.5 rounded-lg border border-border-subtle bg-surface-2/40 p-2.5">
          ...
          <p className="mb-2 text-[11px] text-text-faint">Empty — shorts flow for your whole watchlist. Add names to restrict.</p>
        )}
        <div className="flex gap-2">
          <input
            value={input}
            ...
            onKeyDown={(e) => { if (e.key === "Enter") { e.preventDefault(); addSymbol(); } }}
            placeholder="Add a symbol — type or pick from your watchlist"
            list="short-watchlist"
            className="flex-1 rounded-lg border border-border-subtle bg-surface-2 px-3 py-2 text-[13px] text-text-primary placeholder:text-text-faint outline-none focus:border-accent"
          />
          <datalist id="short-watchlist">
            {wlSuggestions.map((s) => <option key={s} value={s} />)}
          </datalist>
...
function ORBAllowlistSection() {
  ...
  const isAdmin = (me?.email ?? "").trim().toLowerCase() === ORB_ADMIN_EMAIL;
  if (isError || !isAdmin) return null;

  const symbols = (cfg?.orb_symbols || "").split(",").map((s) => s.trim().toUpperCase()).filter(Boolean);
  const wlSuggestions = (watchlist ?? []).map((w) => w.symbol.toUpperCase()).filter((s) => !symbols.includes(s));
  const addSymbol = () => {
    const s = input.trim().toUpperCase();
    setInput("");
    if (!s || symbols.includes(s)) return;
    update.mutate({ orb_symbols: [...symbols, s].join(",") });
  };
  const removeSymbol = (s: string) => update.mutate({ orb_symbols: symbols.filter((x) => x !== s).join(",") });

  return (
    <Section title="ORB names — 15m OR + PDH/PDL (trial)" icon={<Crosshair className="h-4 w-4 text-accent" />}>
      <p className="text-[12px] leading-relaxed text-text-muted mb-3">
        The <b>ORB</b> family — break · held · retest · exit on the 15-minute opening range (ORH/ORL) plus
        PDH/PDL — is being trialed, so it only fires for the names below. Enable the <b>ORB</b> types in
        Alert Types above, then add a few names here to keep the noise controlled. One shared list (admin).
      </p>
      <div className="mb-1.5 flex items-baseline justify-between">
        <label className="text-[11px] font-semibold uppercase tracking-wide text-text-faint">ORB fires only for these</label>
        {symbols.length > 0 && <span className="text-[10px] text-text-faint">{symbols.length}</span>}
      </div>
      {symbols.length > 0 ? (
        <div className="mb-2 flex flex-wrap gap-1.5 rounded-lg border border-border-subtle bg-surface-2/40 p-2.5">
        ...
        <p className="mb-2 text-[11px] text-text-faint">No symbols yet — ORB fires for nobody until you add names.</p>
      )}
      <div className="flex gap-2">
        <input
          value={input}
