...
export interface MarketReportsResponse {
  ...
  breakout_patterns: MarketReport | null;
}
...
export interface LeapDeskRow {
  ...
  sma200: number | null; sma50: number | null; dist_200_pct: number | null; at_200: boolean;
  ...
  strike: number; strike_itm: number; strike_target: number; target_basis: string; dte: number; expiry: string;
...
export function useOpenBracketAllowlist() {
  return useQuery({
    queryKey: ["open-bracket-allowlist"],
    queryFn: () => api.get<OpenBracketAllowlist>("/settings/open-bracket-allowlist"),
    staleTime: 60_000,
  });
}
...
export function useOptionChain() {
  return useMutation({
    mutationFn: (v: { symbol: string; exp: string; type: string; band?: number; near?: number }) => {
      ...
      if (v.near != null) p.set("near", String(v.near));
