...
def _is_collapse_exempt(alert_type_full: str) -> bool:
    ...
    return base in _COLLAPSE_EXEMPT_BASE or base in _RC_VALIDATION_TYPES or base in _FOURH_TYPES or base.startswith("rc_4h")
...
def _check_entry_time_dedup(
    ...
    if base in _FOURH_TYPES and (direction or "").upper() in ("SHORT", "SELL"):
        # Only SPY (index allowlist) may get a 2nd short/day; every OTHER name is capped at 1
        # (user 2026-08-02: "only spy should get more than 1 short"). Before this, the cap of 2
        # applied to every symbol, so AVGO/PYPL/SNDK etc. each got 2 shorts on 07-31.
        _multi = frozenset(s.strip().upper() for s in _envf("V2_MULTI_SHORT_SYMS", "SPY").split(",") if s.strip())
        max_shorts = 2 if (symbol or "").upper() in _multi else 1
...
# Users who opt into "Master Alerts" (Settings) receive the WHOLE master-watchlist
# deduped feed, gated by THIS set instead of their personal type toggles — so a
# brand-new user gets the platform's best signals with one tap (zero-config).
# Drafted from the admin's live curated config (2026-06-30): RC reclaims + weekly/
