...
def _build_batch_prompt(symbols_data: list[dict]) -> str:
    ...
    prompt = (
        ...
        "- Weekly high / low\n"
        "- Monthly high / low\n\n"
        "SWING-SPECIFIC PATTERNS to recognize (spec 60 — these are the\n"
        "rule-based setups the Python swing scanner detects deterministically;\n"
        "the AI version should consider them too):\n"
        "- MA BOUNCE: day's low tagged a key MA (21/50/200 EMA or SMA) and\n"
        "  closed above it, inside a bullish MA stack (50 > 100 > 200).\n"
        "- EMA 8/21 CROSSOVER: daily 8 EMA just crossed above the 21 EMA\n"
        "  with price above the 50 EMA (momentum flip, no chasing).\n"
        "- GOLDEN-CROSS RETEST: 50 EMA crossed above 200 EMA in last ~30\n"
        "  bars AND today's low retested the 50 EMA before closing above.\n"
        "- 52-WEEK-HIGH RETEST: broke a new 52w high in last ~10 bars,\n"
        "  today's pullback retested the prior 52w-high level (now support)\n"
        "  and closed above. Classic continuation on grinders.\n"
        "- 5-DAY-LOW RECLAIM: wicked below the 5-day low and closed back\n"
        "  above with ≥ 1.2× average volume. Price-based mean reversion.\n"
        "- RSI(14) RECOVERY: closed back above 30 after being ≤ 30 within\n"
        "  the last 7 bars. Oversold reversal.\n"
        "When you detect any of these patterns, prefer them over generic\n"
        "near-MA proximity. Tag the setup_type with the specific pattern\n"
        "name (e.g., 'Golden-cross retest', '5-day-low reclaim').\n\n"
