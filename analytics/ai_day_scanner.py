...
def build_day_trade_prompt(
    ...
    prompt = (
        ...
        "   Entry = PDL level. Stop = below session low (must be at least 0.3% below entry).\n\n"
        "2. PDH BREAKOUT ON VOLUME: Price closes above prior day high (PDH) with volume >= 1.5x average.\n"
        "   Entry = PDH level. Stop = below breakout bar low (must be at least 0.3% below entry).\n\n"
        "3. VWAP HOLD: Price was above VWAP earlier, pulled back to VWAP, 2-3 bars close above VWAP.\n"
        "   Entry = VWAP level. Stop = session low or VWAP minus at least 0.3% of price.\n\n"
        "4. DOUBLE BOTTOM HOLD: Price tests the same low from a prior session (within 0.5%).\n"
        "   Second touch holds (2+ bars above). Entry = double bottom level. Stop = below the level (at least 0.3%).\n\n"
        "5. MA/EMA HOLD: Price touches 20/50/100/200 MA or EMA and bounces.\n"
        "   Bar low touches MA, next 2-3 bars close above. Entry = MA level. Stop = below MA (at least 0.3%).\n\n"
        "STOP RULES:\n"
        "- Stop = below the SUPPORT LEVEL that defines the trade (not just below entry)\n"
        "- PDL trade: stop = below session low or PDL level\n"
        "- VWAP trade: stop = below session low (VWAP break = thesis broken)\n"
        "- MA trade: stop = below the MA level\n"
        "- Stop must be AT LEAST 0.3% below entry. $0.30 on $2200 is NOT a valid stop.\n"
        "- For ETH: min stop ~$7. For BTC: min ~$220. For SPY: min ~$2.\n\n"
