...
def build_day_trade_prompt(
    ...
    prompt = (
        ...
        "  (c) volume picking up on the bounce bar (>0.7x avg, not required at exactly 1.0x)\n"
        "  (d) multi-level confluence (price at 2+ levels: e.g. VWAP + 50MA, PDL + 100EMA)\n"
        "  (e) reclaim pattern (price briefly broke level, now back above)\n"
        ...
        "  (d) multi-level confluence (resistance stack)\n"
        "  (e) failed breakout (price briefly pushed through, now back below)\n"
        ...
        "- RSI >= 70 at ANY resistance level (overbought confluence makes it a valid short)\n"
        "NEVER short these (they're strength, not weakness):\n"
        "- Session high break or hold with volume → this is LONG continuation, never SHORT\n"
        "- Session high first-test without structural level confluence → RESISTANCE notice only, NOT SHORT trade\n"
        "- Breakouts through prior day / session levels on volume → LONG, not SHORT\n"
        "Day-structure check:\n"
        "- If intraday shows higher highs + higher lows (uptrending day) AND price above VWAP,\n"
        "  do NOT call session high RESISTANCE either. Session highs during uptrends are\n"
        "  continuation levels — prefer LONG pullback entries or WAIT.\n\n"
