...
# ── Page Setup ──────────────────────────────────────────────────────────────
user = ui_theme.setup_page("dashboard", tier_required="free")
...
_TF_CONFIG = {
    "5m": {"period": "60d", "interval": "5m", "date_fmt": "%H:%M", "label": "5m"},
    "15m": {"period": "60d", "interval": "15m", "date_fmt": "%b %d %H:%M", "label": "15m"},
    "1h": {"period": "2y", "interval": "1h", "date_fmt": "%b %d %H:%M", "label": "1H"},
    "Daily": {"period": "2y", "interval": "1d", "date_fmt": "%b %d", "label": "D"},
    "Weekly": {"period": "5y", "interval": "1wk", "date_fmt": "%b %Y", "label": "W"},
}

_EMA_COLORS = {"EMA 20": "#1abc9c", "EMA 50": "#f39c12", "EMA 100": "#3498db", "EMA 200": "#e74c3c"}

_sym_col, _tf_col = st.columns([2, 3])
with _sym_col:
...
with _tf_col:
    _chart_tf = st.radio(
        "Timeframe", list(_TF_CONFIG.keys()), index=0,
        horizontal=True, label_visibility="collapsed", key="dash_chart_tf",
    )

_tf = _TF_CONFIG[_chart_tf]

# Fetch bars based on timeframe
@st.cache_data(ttl=300, show_spinner=False)
def _fetch_tf_bars(sym: str, period: str, interval: str) -> pd.DataFrame:
    return fetch_ohlc(sym, period=period, interval=interval)

_chart_bars = _fetch_tf_bars(_chart_sym, _tf["period"], _tf["interval"])

if not _chart_bars.empty:
    _chart_prior = _cached_prior_day(_chart_sym)
    x_int = list(range(len(_chart_bars)))
    time_labels = _chart_bars.index.strftime(_tf["date_fmt"])
    ...
    fig = ui_theme.build_candlestick_fig(
        ...
        height=560, tick_vals=tick_vals, tick_text=tick_text,
    )

    # EMA overlays (computed on the chart bars themselves)
    for ema_name, ema_color in _EMA_COLORS.items():
        ema_period = int(ema_name.split()[-1])
        if len(_chart_bars) >= ema_period:
            ema_series = _chart_bars["Close"].ewm(span=ema_period, adjust=False).mean()
            fig.add_trace(go.Scatter(
                x=x_int, y=ema_series.values,
                mode="lines", name=ema_name,
                line=dict(color=ema_color, width=1.3),
            ), row=1, col=1)

    # VWAP (intraday only)
    if _chart_tf in ("5m", "15m"):
    ...
    # Prior day levels (intraday only)
    if _chart_tf in ("5m", "15m", "1h") and _chart_prior:
        ...
        for sig in all_signals:
            ...
            if sig.entry and sig.direction in ("BUY", "SHORT"):
                ...
                if _ack_active and is_new:
                    ...
                    if _sig_alert_id:
                        if col1.button("Track This", key=f"ack_{sig.symbol}_{sig.alert_type.value}", type="primary"):
...
# ── Alert History (tabbed: Live / Session Log / History) ─────────────────────
st.markdown("<div class='tc-section'>Alert History</div>", unsafe_allow_html=True)

from ui_theme import get_current_tier, render_inline_upgrade, render_alert_card
from alerts_pdf import generate_alerts_pdf

_dash_tier = get_current_tier()
_dash_is_free = _dash_tier == "free"
_dash_uid = None if _dash_tier == "admin" else user["id"]

dash_tab_live, dash_tab_session, dash_tab_history = st.tabs(
    ["Live", "Session Log", "History"]
)

# ── Tab 1: Live (today's alert table — original home.py behavior) ──
with dash_tab_live:
...
# ── Tab 2: Session Log (today's summary KPIs) ──
with dash_tab_session:
    _today_summary = get_session_summary(user_id=user["id"])
    if _today_summary["total"] > 0:
        sc1, sc2, sc3, sc4, sc5, sc6 = st.columns(6)
        sc1.metric("Total", _today_summary["total"])
        sc2.metric("Entries", _today_summary["buy_count"])
        sc3.metric("Exits", _today_summary["sell_count"])
        sc4.metric("T1 Hits", _today_summary["t1_hits"])
        sc5.metric("T2 Hits", _today_summary["t2_hits"])
        sc6.metric("Stopped", _today_summary["stopped_out"])

        if _today_summary.get("signals_by_type"):
            type_df = pd.DataFrame([
                {"Type": k.replace("_", " ").title(), "Count": v}
                for k, v in _today_summary["signals_by_type"].items()
            ]).sort_values("Count", ascending=False)
            st.dataframe(type_df, use_container_width=True, hide_index=True)
    else:
        st.caption("No alerts in this session yet.")

# ── Tab 3: History (multi-day — absorbed from 9_Alerts.py) ──
with dash_tab_history:
    _hist_session_dates = get_session_dates(user_id=_dash_uid)

    if not _hist_session_dates:
        ui_theme.empty_state("No alert history yet.")
    elif _dash_is_free:
        _hist_dates_to_show = _hist_session_dates[:1]
        st.caption(f"Showing latest session (free tier) · {len(_hist_session_dates)} total")
        render_inline_upgrade("Full alert history — browse all sessions", "pro")

        # Show latest session alerts
        _h_summary = get_session_summary(session_date=_hist_dates_to_show[0], user_id=_dash_uid)
        for _ha in _h_summary.get("alerts", []):
            render_alert_card(_ha)
    else:
        import datetime as _dt

        _hist_date_objs = []
        for ds in _hist_session_dates:
            try:
                _hist_date_objs.append(_dt.date.fromisoformat(ds))
            except Exception:
                pass

        if _hist_date_objs:
            _hdp_col, _hmode_col = st.columns([1, 2])
            with _hdp_col:
                _h_picked = st.date_input(
                    "Session date",
                    value=_hist_date_objs[0],
                    min_value=_hist_date_objs[-1],
                    max_value=_hist_date_objs[0],
                    key="dash_hist_date",
                )
            with _hmode_col:
                _h_view_mode = st.radio(
                    "Quick range",
            ...
            _h_picked_str = str(_h_picked)
            if _h_view_mode == "Single day":
                if _h_picked_str in _hist_session_dates:
                    _hist_dates_to_show = [_h_picked_str]
                else:
                    st.warning(f"No alerts on {_h_picked_str} — showing nearest session")
                    _hist_dates_to_show = _hist_session_dates[:1]
            elif _h_view_mode == "Last 3 days":
                _hist_dates_to_show = _hist_session_dates[:3]
            elif _h_view_mode == "Last 7 days":
                _hist_dates_to_show = _hist_session_dates[:7]
            else:
                _hist_dates_to_show = _hist_session_dates
        else:
            _hist_dates_to_show = _hist_session_dates[:1]

        st.caption(f"{len(_hist_dates_to_show)} session(s) · {len(_hist_session_dates)} total")

        # Collect alerts across selected dates
        _h_all_alerts: list[dict] = []
        _h_summaries: dict[str, dict] = {}
        for d in _hist_dates_to_show:
            s = get_session_summary(session_date=d, user_id=_dash_uid)
            _h_summaries[d] = s
            for a in s["alerts"]:
                a["_session_date"] = d
                _h_all_alerts.append(a)

        if not _h_all_alerts:
            ui_theme.empty_state("No alerts in the selected date range.")
        else:
            # Global KPIs
            _h_total_buy = sum(s["buy_count"] for s in _h_summaries.values())
            _h_total_sell = sum(s["sell_count"] for s in _h_summaries.values())
            _h_total_t1 = sum(s["t1_hits"] for s in _h_summaries.values())
            _h_total_t2 = sum(s["t2_hits"] for s in _h_summaries.values())
            _h_total_stopped = sum(s["stopped_out"] for s in _h_summaries.values())

            hc1, hc2, hc3, hc4, hc5, hc6 = st.columns(6)
            hc1.metric("Total", len(_h_all_alerts))
            hc2.metric("Entries", _h_total_buy)
            hc3.metric("Exits", _h_total_sell)
            hc4.metric("T1 Hits", _h_total_t1)
            hc5.metric("T2 Hits", _h_total_t2)
            hc6.metric("Stopped", _h_total_stopped)

            # Filters
            _hf_cols = st.columns(3)
            with _hf_cols[0]:
                _h_all_syms = sorted(set(a["symbol"] for a in _h_all_alerts))
                _h_sel_syms = st.multiselect("Symbol", _h_all_syms, default=None,
                                             placeholder="All symbols", key="dash_hist_sym")
                if not _h_sel_syms:
                    _h_sel_syms = _h_all_syms
            with _hf_cols[1]:
                _h_all_dirs = sorted(set(a["direction"] for a in _h_all_alerts))
                _h_sel_dirs = st.multiselect("Direction", _h_all_dirs, default=None,
                                             placeholder="All directions", key="dash_hist_dir")
                if not _h_sel_dirs:
                    _h_sel_dirs = _h_all_dirs
            with _hf_cols[2]:
                _h_all_types = sorted(set(a["alert_type"] for a in _h_all_alerts))
                _h_sel_types = st.multiselect("Alert Type", _h_all_types, default=None,
                                              placeholder="All types", key="dash_hist_type",
                                              format_func=lambda t: t.replace("_", " ").title())
                if not _h_sel_types:
                    _h_sel_types = _h_all_types

            _h_filtered = [
                a for a in _h_all_alerts
                if a["symbol"] in _h_sel_syms
                and a["direction"] in _h_sel_dirs
                and a["alert_type"] in _h_sel_types
            ]

            st.caption(f"Showing {len(_h_filtered)} of {len(_h_all_alerts)} alerts")

            # PDF export
            _h_pdf_bytes = generate_alerts_pdf(_h_filtered, _h_summaries, _hist_dates_to_show)
            st.download_button(
                label="Download PDF Report",
                data=_h_pdf_bytes,
                file_name=f"tradecopilot_alerts_{_hist_dates_to_show[0]}.pdf",
                mime="application/pdf",
                key="dash_hist_pdf",
            )

            # Render alert cards grouped by date
            from collections import defaultdict as _defaultdict
            _h_grouped: dict[str, list[dict]] = _defaultdict(list)
            for a in _h_filtered:
                _h_grouped[a["_session_date"]].append(a)

            for _h_session_date in _hist_dates_to_show:
                _h_day_alerts = _h_grouped.get(_h_session_date, [])
                if not _h_day_alerts:
                    continue

                _h_date_display = pd.Timestamp(_h_session_date).strftime("%A, %b %d %Y")
                _h_buy_c = sum(1 for a in _h_day_alerts if a.get("direction") == "BUY")
                _h_sell_c = sum(1 for a in _h_day_alerts if a.get("direction") in ("SELL", "SHORT", "NOTICE"))
                ...
                st.markdown(
                    ...
                    f"<span style='font-weight:700;font-size:1.1rem'>{_h_date_display}</span>"
                    f"<span style='font-size:0.85rem;color:#8b949e'>"
                    f"{len(_h_day_alerts)} alerts · "
                    f"<span style='color:#3fb950'>{_h_buy_c} Entry</span> · "
                    f"<span style='color:#f85149'>{_h_sell_c} Exit</span>"
                    f"</span>"
                    f"</div>",
                    unsafe_allow_html=True,
                )

                for _ha in _h_day_alerts:
                    render_alert_card(_ha)
