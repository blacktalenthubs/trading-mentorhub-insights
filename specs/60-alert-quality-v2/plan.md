...
**P2b. PDH / PWH / PMH break with volume.** Three new alert types
re-introducing the rule family spec 58 retired, **gated by volume so the
weak fakeouts spec 58 was right to fear cannot fire**:

```
staged_pdh_break_vol:
   close > pdh
   AND close[1] <= pdh        ← just broke this bar (no chasing)
   AND volume_ratio >= 2.0    ← non-negotiable quality gate
   AND uptrend_pass
```

Identical logic for `staged_pwh_break_vol` (weekly high) and
`staged_pmh_break_vol` (monthly high). Catalog default-disabled; user
opts in.
...
| FR-006 | New: staged_pdh_break_vol / pwh_break_vol / pmh_break_vol — close just broke level, volume ≥ 2.0×, uptrend_pass | P2 |
| FR-007 | New: gap_up_continuation_long — open > PDH, close > PDH, vol ≥ 1.5×; stop = first-15m low | P2 |
